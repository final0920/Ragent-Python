...
均默认关闭,`uv sync --group infra` 后启用:

- **对象存储(S3/MinIO)**:`.env` 设 `S3_ENABLED=true`(`docker compose up` 已含 minio,控制台 :9001)。上传文档原文件入 S3,`GET /knowledge-base/docs/{id}/file` 取预签名下载链接。
- **MQ 异步(Celery)**:`.env` 设 `CELERY_ENABLED=true`,另起 worker:`uv run celery -A app.celery_app worker -P solo -l info`。上传后分块入库异步执行,接口立即返回 `status=processing`,worker 完成后置 `done`。
